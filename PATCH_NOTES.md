...
### 🚀 Nouveautés
- Ajoute la commande `/note` pour consigner rapidement une observation de terrain (état sanitaire, arrosage, paillage, remarque générale) — l'assistant guide la saisie en demandant les informations pertinentes selon la catégorie choisie ; aussi accessible en dictant "je veux noter une observation" (US-038)
- Un récapitulatif s'affiche avant tout enregistrement, avec boutons Confirmer/Annuler (US-038)
- Affiche les observations dictées au bot directement dans le dashboard : une icône bulle de dialogue apparaît sur la parcelle (Plan) ou sur la culture (Stocks) selon que la note est localisée ou non, avec un aperçu dépliable au clic, paginé par blocs de 3 et un badge indiquant le nombre de notes (plafonné à "10+") (US-039)
- Une note liée à une culture et une variété précises, mais sans parcelle indiquée, est automatiquement rattachée à la bonne parcelle dans Plan si cette culture/variété n'y est actuellement cultivée qu'à un seul endroit ; sinon elle reste sur l'agrégat Stocks (US-039)
...
- Corrige l'API des observations qui retournait un texte nettoyé de sa catégorie — `GET /observations` renvoie maintenant le `commentaire` brut (préfixe `[Catégorie]` inclus), sans interprétation (US-039)
...
- Nouvel endpoint `GET /observations`, et `GET /plan` / `GET /stats` enrichis d'indicateurs `has_observations` / `nb_observations` (US-039)
