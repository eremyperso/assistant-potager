...
| `days_to_harvest` (bis) | ✅ retenue → durée `plantation_recolte` [US-177, 16/09/2026], **à l'abri ou pour ce qui ne se sème pas** | Le motif qui l'écarte de `recolte` la qualifie ici : elle compte depuis la plantation. Retenue quand le mode voté est « start indoors » (tomate, poivron, aubergine, thym) ou « ne se sème pas » (ail, échalote, pomme de terre, framboise) — **8 cultures** sur l'extrait versionné. Jamais pour un semis en place ni un mode indéterminé, et jamais obtenue en retranchant `repiquage` à `recolte` (`adaptateur_wind_river._retenir_plantation_recolte`) |
