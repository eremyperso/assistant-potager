...
| `parcelles-et-plan.md` | les parcelles (renommage, suppression logique, `est_pepiniere`, abri et paillage déclarés — US-181, le **nombre de rangs** déclaré d'une planche — US-197, la **longueur** d'une planche et la largeur qui s'en déduit — US-225), le plan d'occupation, la **répartition des cultures en rangs** — rangs occupés et restants, quantité par rang, mode d'implantation, numérotation dans l'ordre d'installation, dépassement, lots d'une pépinière (`app/services/repartition_rangs.py`, US-198), l'**espacement sur le rang** dérivé du référentiel et les rangs sans places calculées (`app/services/espacement_rang.py`, US-226), la **phase du moment** d'une culture en place (`recalage_calendrier.phase_de_serie`, US-194), la **lecture de la Vue plan** — formes, couleurs, rangs libres, numérotation, parcelle sans nombre de rangs (`frontend/src/lib/planVue.js`, `frontend/src/views/PlanVue.jsx`, US-200), les **places d'un rang** — capacité, places prises et restantes, dépassement, semis en ligne, capacité d'exemple d'un rang libre (`app/services/repartition_rangs.py`, US-227) et leur **dessin en piste de places** — sept symboles proportionnels, emoji de culture et pousses en contour, phase écrite et pictogrammes de phase, mode dégradé (`frontend/src/components/ui/PisteDesPlaces.jsx`, US-228), la **fiche d'une parcelle et ses caractéristiques** — nom, superficie, longueur, largeur déduite, rangs, exposition, type de sol, abri, paillage, pépinière, statut, et ce qui manque nommé plutôt que masqué (`frontend/src/lib/planParcelles.js`, US-229), la **correction de ces caractéristiques depuis le web** — carte éditable sur place, un seul enregistrement, « Non renseigné » qui efface, largeur non saisissable, bornes et messages de refus partagés avec le compagnon, renommage / bascule pépinière / passage en inactive annoncés (`app/services/parcelles.py#modifier_parcelle`, `PATCH /parcelles/{id}`, US-230), la **rotation d'une parcelle année par année** — trois campagnes passées, familles conseillées pour la campagne à venir, alerte de répétition, famille non renseignée exclue du calcul, teinte d'identité par famille, pas de rotation sur une pépinière (`app/services/rotation.py#historique_du_plan`, `frontend/src/lib/familles.js`, US-231), la **fiche d'une parcelle sans le détail de ses cultures** — bandeau d'occupation, noms des cultures, « Voir les cultures dans le Plan », retrait des tuiles, des rangs libres actionnables, de la frise et de la confiance (`frontend/src/views/Plan.jsx`, amendement d'US-222 du 24/09/2026), le **journal du sol d'une parcelle** — paillage, amendement, désherbage, binage filtrés et regroupés par planche, huit dernières interventions, renvoi au Journal filtré, gestes de sol ouverts à la PWA, catégories de la maquette sans équivalent (`app/services/evenements.interventions_sol`, `GESTES_SOL`, US-232), ce que l'interpréteur de commandes rend dictable sur les parcelles (`app/services/interpreteur_commandes.py`) |
...
| `cultures-familles-et-rotation.md` | la famille botanique et son délai de retour, le calcul de rotation, l'avertissement à la plantation, les associations (la **lecture à froid** de cette même rotation, sur la fiche d'une parcelle, est décrite dans `parcelles-et-plan.md` — US-231) |
