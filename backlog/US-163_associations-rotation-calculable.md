...
- **Arbitrage tranché — associations saisies, pas importées :** conséquence directe de l'option A sur la licence. Le coût est réel et il est assumé ; le périmètre initial de dix cultures le rend borné
  - ⚠️ **Amendement du 02/09/2026, décidé par le porteur produit :** l'arbitrage visait l'absence de source non contaminante — hypothèse fausse pour `wind_river_greens` (CC BY 4.0, déjà au socle depuis US-161). `companion_plants.csv`, déjà extrait brut pour cette même source, est donc curé et traduit (`app.services.adaptateur_wind_river.curer_associations`) plutôt que rejeté : 113 des 217 arêtes retenues, après retrait de ce qui est hors périmètre (ornementales, arbres), des 8 motifs recyclés et de l'auto-association relevés par l'audit du 01/09/2026, et une nouvelle contradiction démasquée par la fusion des doublons de libellé. **La saisie au bot reste le chemin premier** (CA10) — l'import ne fait qu'ajouter une seconde source possible, avec la même règle de non-écrasement qu'ailleurs dans le référentiel (une correction du jardinier n'est jamais réécrite par un rejeu). Voir `data/referentiel/wind_river_greens/SOURCE.md`
