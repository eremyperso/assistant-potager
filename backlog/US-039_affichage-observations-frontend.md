...
- *(Révisé)* Le texte affiché reprend le `commentaire` brut, préfixe `[Catégorie]` inclus — la maquette affichait un texte nettoyé, mais l'info de catégorie a été jugée trop utile pour être perdue côté API
...
- [ ] CA3 : Un nouvel endpoint `GET /observations` expose la liste `{date, texte}` (`texte` = `commentaire` brut) triée du plus récent au plus ancien, filtrable par `parcelle_id` seul, par `culture` seule, ou par `parcelle_id`+`culture`+`variete`
...
- [ ] CA8 *(révisé)* : L'API `/observations` renvoie le `commentaire` brut, préfixe `[Catégorie]` inclus, sans interprétation côté backend — l'info de catégorie ne doit pas être perdue en transit
...
- *(Révisé)* Aucune extraction/nettoyage du préfixe catégorie n'est plus effectuée — `commentaire` est renvoyé tel quel par l'API
