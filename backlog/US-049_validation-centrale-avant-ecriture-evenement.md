...
- [x] CA1 : Une fonction unique de validation (ex. `valider_evenement(db, ctx, culture, variete, parcelle_obj, action) -> None`, lève une exception métier explicite si invalide) est appelée systématiquement à l'intérieur de **chacune** des fonctions de `app/services/evenements.py` qui écrivent un `Evenement`, jamais dans `bot.py` ou `main.py`
- [x] CA2 : La validation regroupe au minimum les règles déjà identifiées : (a) culture jamais introduite via semis/plantation/mise_en_godet pour les actions qui présupposent une culture existante ; (b) incohérence culture/variété ↔ parcelle citée quand une parcelle est fournie ; (c) parcelle inconnue — migrée dans `valider_evenement` (paramètre `nom_parcelle_brut`) pour le chemin `creer_evenement_confirme` (seul chemin qui bloquait déjà dur sur ce cas) ; les chemins `creer_evenement_depuis_parse`/`creer_evenement_ligne` (API `/parse`) et `creer_evenement_observation` (notes) conservent leur comportement lenient existant (parcelle_id NULL plutôt que blocage), non modifié pour respecter CA6
- [x] CA3 : Aucune des règles ne dépend du nombre d'items traités dans un même appel Telegram — la validation s'applique item par item, indépendamment de tout `len(items) == 1` côté appelant
- [x] CA4 : Un test d'intégration reproduit exactement le bug rapporté (phrase multi-culture segmentée par Groq en plusieurs items dans la même réponse) et prouve qu'aucun des deux événements incohérents n'est écrit
- [x] CA5 : Un test générique parcourt automatiquement toutes les fonctions de `app/services/evenements.py` qui construisent un `Evenement` (par introspection ou liste explicite documentée) et vérifie qu'aucune ne permet d'écrire une culture jamais plantée — pour empêcher qu'une future fonction d'écriture oublie d'appeler la validation
- [x] CA6 : Le comportement observable pour l'utilisateur reste celui déjà livré sur `fix/telegram-recolte-validation` (parcelle incohérente traitée comme non-saisie, culture inconnue bloquée avec message explicite) — cette US est un refactor de robustesse, pas un changement de comportement fonctionnel
- [x] CA7 : `deplacer_evenements` (réassociation culture/parcelle a posteriori) et `corriger_evenement` (flux de correction conversationnel) passent par la même validation avant toute mise à jour, pas seulement les créations

**Note CA1** (à date) : `parcelle_id` inconnue (règle c) reste par ailleurs contrôlée séparément dans `_do_save_items` (résolution + blocage si aucune parcelle ne correspond au nom cité) — cette règle-là n'est pas encore migrée dans `valider_evenement` car elle porte sur la résolution du nom brut, pas sur la cohérence d'un objet `Parcelle` déjà résolu ; à évaluer dans une itération suivante si jugé utile.
