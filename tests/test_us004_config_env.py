...
def _reload_config(monkeypatch, env_name, env_vars):
    ...
    # Forcer le rechargement du module config — et restaurer ensuite l'attribut
    # `app.config` du package, sinon les tests suivants verraient ce module-ci.
    import app as _app
    monkeypatch.setattr(_app, "config", getattr(_app, "config", None), raising=False)
    if "app.config" in sys.modules:
        monkeypatch.delitem(sys.modules, "app.config")
    config = importlib.import_module("app.config")  # ré-exécute le module (l'attribut app.config resterait en cache)
    return config
